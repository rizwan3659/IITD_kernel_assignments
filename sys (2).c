...
{
	long ret;
        buff=(char*) krealloc(buff,QUEUE_SIZE*sizeof(char),GFP_KERNEL);
	if(!buff)
	{
	...
	down(&empty);
	down_write(&rwsem);
	ret = __copy_from_user(buff+in,arr_user,sizeof(char)*8);
	if(ret)
	{
...
SYSCALL_DEFINE1(reader,char*,arr_reader)
{
	...
	down(&full);
	down_read(&rwsem);
	ret= __copy_to_user(arr_reader,buff+out,sizeof(char)*8);
	if(ret)
	{
		return -EFAULT;
	}
