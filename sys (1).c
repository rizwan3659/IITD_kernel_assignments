...
SYSCALL_DEFINE1(writer,char*,arr_user)
{
	...
	//if(buff==NULL)
	//{
        buff=(char*) krealloc(buff,128*sizeof(char),GFP_KERNEL);
	if(!buff)
	{
		printk(KERN_INFO "buffer error");
		return -EFAULT;
	}
	printk(KERN_INFO "buffer pointer--%p",buff);
	//memset(buff,0,sizeof(char)*128);
	//printk(KERN_INFO "INSIDE WRITER1");
	if(!arr_user)
	{
		printk(KERN_INFO "arr_user error");
		return -EFAULT;
	}
	//printk(KERN_INFO "INSIDE WRITER2");
	ret = __copy_from_user(buff+counter,arr_user,sizeof(char)*8);
	...
	if(!buff)
	{
		printk(KERN_INFO "buffer error_1");
		return -EFAULT;
	}
	if(!arr_user)
	{
		printk(KERN_INFO "arr_user error_1");
		return -EFAULT;
	}
	//printk(KERN_INFO "INSIDE WRITER3");	
	printk(KERN_INFO "buffer-%s",buff);
        printk(KERN_INFO "after buffer");
	printk(KERN_INFO "after buffer1 ");
	//}
	return 0;
}
SYSCALL_DEFINE1(reader,char*,arr_reader)
{
	long ret;
	printk(KERN_INFO "buffer-%s\n",buff);
	ret= __copy_to_user(arr_reader,buff,sizeof(char)*8);
	if(ret)
	{
		return -EFAULT;
	}
